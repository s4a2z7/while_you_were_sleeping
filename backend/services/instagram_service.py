...
class InstagramService:
    ...
    def __init__(self):
        ...
        if not self.username or not self.password:
            logger.warning("⚠️  인스타그램 계정 정보가 설정되지 않았습니다.")
        ...
        try:
            ...
            logger.error(f"❌ 인스타그램 클라이언트 초기화 실패: {e}")
            self.client = None
    ...
    def login(self) -> bool:
        ...
        try:
            if not self.client or not self.username or not self.password:
                logger.error("❌ 인스타그램 설정이 불완전합니다.")
                return False
            
            logger.info(f"🔐 인스타그램 로그인 중 ({self.username})...")
    ...
    def post_image(self, image_path, caption: str) -> bool:
        ...
        try:
            if not self.client:
                logger.error("❌ 인스타그램 클라이언트가 초기화되지 않았습니다.")
                return False
            ...
            media = self.client.photo_upload(str(image_path), caption=caption)
            logger.info(f"✅ 인스타그램 발행 완료 (ID: {media.pk})")
            return True
    ...
    def run(self) -> bool:
        ...
        try:
            ...
            logger.error(f"❌ 인스타그램 서비스 오류: {str(e)}")
            import traceback
...
def main():
    """메인 실행 함수"""
    logger.info("=" * 60)
    logger.info("🚀 인스타그램 자동 발행 서비스 시작")
    logger.info("=" * 60)
    ...
    if service.run():
        logger.info("=" * 60)
        logger.info("✅ 인스타그램 발행 완료")
        logger.info("=" * 60)
        sys.exit(0)
    else:
        logger.error("=" * 60)
        logger.error("❌ 인스타그램 발행 실패")
        logger.error("=" * 60)
        sys.exit(1)
